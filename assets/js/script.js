          ...
          concertsContainer.innerHTML= ""
            ...
            for(var i = 0; i < eventsArr.length; i ++){
               var events = document.createElement("a");
                events.setAttribute("style", "border-radius: 10px; background-color: #06b6d4; color: white; height: 200px; width: 100%; border: 2px solid black;")
                events.href = eventsArr[i].url;
                events.textContent = eventsArr[i].name +"  -  "+ eventsArr[i].dates.start.localDate.substring(5,10) +"-"+ eventsArr[0].dates.start.localDate.substring(0,4)
                events.style.backgroundImage = "url('"+eventsArr[i].images[0].url+"')"
